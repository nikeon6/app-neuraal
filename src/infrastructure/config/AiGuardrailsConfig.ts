...
export function getAiGuardrailsConfig(): AiGuardrailsConfig {
  return {
    summary: {
      ...
      maxInputBytes: 0,
      monthlyQuotaRequests: parseEnvInt("AI_SUMMARY_MONTHLY_QUOTA_REQUESTS", 100, 1),
      monthlyQuotaTokens: parseEnvInt("AI_SUMMARY_MONTHLY_QUOTA_TOKENS", 0, 0),
    },
    transcriptYoutube: {
      rateLimitPerMinute: parseEnvInt("AI_TRANSCRIPT_RATE_LIMIT_PER_MINUTE", 3, 1),
      rateLimitPerHour: 0,
      maxActivePerUser: parseEnvInt("AI_TRANSCRIPT_MAX_ACTIVE_PER_USER", 1, 1),
      maxActivePerEntry: parseEnvInt("AI_TRANSCRIPT_MAX_ACTIVE_PER_ENTRY", 1, 0),
      maxInputChars: parseEnvInt("AI_TRANSCRIPT_MAX_INPUT_CHARS", 12_000, 100),
      maxInputBytes: 0,
      monthlyQuotaRequests: parseEnvInt("AI_TRANSCRIPT_MONTHLY_QUOTA_REQUESTS", 50, 1),
      monthlyQuotaTokens: parseEnvInt("AI_TRANSCRIPT_MONTHLY_QUOTA_TOKENS", 0, 0),
    },
    ocrImage: {
      rateLimitPerMinute: parseEnvInt("AI_OCR_RATE_LIMIT_PER_MINUTE", 5, 1),
      rateLimitPerHour: 0,
      ...
      monthlyQuotaRequests: parseEnvInt("AI_OCR_MONTHLY_QUOTA_REQUESTS", 200, 1),
      monthlyQuotaTokens: parseEnvInt("AI_OCR_MONTHLY_QUOTA_TOKENS", 0, 0),
    },
    reminderWhatsapp: {
      rateLimitPerMinute: parseEnvInt("AI_WHATSAPP_RATE_LIMIT_PER_MINUTE", 5, 1),
      rateLimitPerHour: 0,
      ...
      maxInputBytes: 0,
      monthlyQuotaRequests: parseEnvInt("AI_WHATSAPP_MONTHLY_QUOTA_REQUESTS", 200, 1),
...
export function getActionConfig(
  ...
  switch (action) {
    case "SUMMARY": return config.summary;
    case "TRANSCRIPT_YOUTUBE": return config.transcriptYoutube;
    case "OCR_IMAGE": return config.ocrImage;
    case "REMINDER_WHATSAPP": return config.reminderWhatsapp;
    default: return null;
  }
}
