...
function guardErrorCodeToStatus(code: string): number {
  switch (code) {
    case "RATE_LIMITED":
      return 429;
    case "QUOTA_EXCEEDED":
...
function summaryErrorCodeToStatus(code: string): number {
  switch (code) {
    case "NOT_FOUND":
      return 404;
    case "CONFLICT":
...
export async function POST(
  ...
  try {
    ...
    if (guardResult.isErr()) {
      ...
      const statusCode = guardErrorCodeToStatus(code);
      return NextResponse.json(
    ...
    if (result.isErr()) {
      ...
      const statusCode = summaryErrorCodeToStatus(code);
      return NextResponse.json(
        { error: { code, message } },
        { status: statusCode },
      );
    ...
    return NextResponse.json(
      {
        ...
        message:
          "Summary generation started. Check notifications for progress.",
      },
      { status: 202 },
    );
  } catch (error) {
