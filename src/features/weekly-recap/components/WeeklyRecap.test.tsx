...
const mockEntriesByDate: Record<string, unknown[]> = {
  "2026-02-02": [
    { id: "1", type: "task", title: "Write report", topicId: "t1", completed: true, date: "2026-02-02" },
    { id: "2", type: "task", title: "Go running", topicId: "t2", completed: false, date: "2026-02-02" },
    { id: "3", type: "note", title: "Meeting notes", topicId: null, completed: null, date: "2026-02-02" },
  ],
  "2026-02-03": [
    { id: "4", type: "task", title: "Team meeting", topicId: "t1", completed: true, date: "2026-02-03" },
  ],
};
const mockTopics = [
...
vi.mock("./CompletionDonutChart", () => ({
  CompletionDonutChart: ({ completed, pending }: { completed: number; pending: number }) => (
    <div data-testid="donut-chart">Donut: {completed}/{pending}</div>
  ),
}));

vi.mock("./TopicBubbleChart", () => ({
  TopicBubbleChart: ({ data }: { data: unknown[] }) => (
    <div data-testid="bubble-chart">Bubbles: {data.length} topics</div>
  ),
}));

vi.mock("./DailyBarChart", () => ({
  DailyBarChart: ({ data }: { data: unknown[] }) => (
    <div data-testid="bar-chart">Bars: {data.length} days</div>
  ),
}));
...
describe("WeeklyRecap", () => {
  ...
  describe("Rendering", () => {
    ...
    it("passes correct completion counts to donut chart (only tasks, not notes)", () => {
      ...
      expect(screen.getByTestId("donut-chart")).toHaveTextContent("2/1");
    });

    it("passes 7 days of data to bar chart", () => {
      renderWithProviders(<WeeklyRecap />);

      expect(screen.getByTestId("bar-chart")).toHaveTextContent("7 days");
    });
    ...
    it("passes only tasks (not notes) to task list", () => {
      ...
      expect(screen.getByTestId("task-list")).toHaveTextContent("Tasks: 3");
    });
    ...
    it("passes topic distribution data to bubble chart", () => {
      ...
      expect(screen.getByTestId("bubble-chart")).toHaveTextContent("2 topics");
    });
  });
});
